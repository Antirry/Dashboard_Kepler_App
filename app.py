...
# Функции передачи параметров из слайдера в функцию, которая строит график и наоборот (график в приложение)
# State - состояние, применяется как триггер обновления (для кнопки)
...
# Список словарей, в каждом из которых идут значения из категорий
options1 = []
for k in names:
    options1.append({"label": k, "value": k})

# Название, настройки (значения категории), значения, несколько значений (Фильтр размера звезд)
...
# Оболочка над функцией используется для динамики (Обращение, отправление параметров и получение результата)
